...
import type { FormularioInspeccionEmergencia, InspeccionExtintor } from '../schemas/inspeccion-emergencia.schema'
@Injectable()
export class InspeccionesEmergenciaExcelService {
  ...
  constructor(private readonly configService: ConfigService) {
    this.templatePath =
      ...
      path.join(process.cwd(), "src", "templates", "Inspeccion_Sistemas_Emergencia_Externos.xlsx")
  }
    ...
    private async insertarImagen(worksheet: ExcelJS.Worksheet, base64Image: string, cellRange: string) {
      ...
      private async llenarInformacionGeneral(worksheet: ExcelJS.Worksheet, inspeccion: FormularioInspeccionEmergencia) {
        try {
      ...
      private async llenarInspeccionesPorMes(worksheet: ExcelJS.Worksheet, inspeccion: FormularioInspeccionEmergencia) {
        try {
          ...
          const mesesSegundoSemestre = ["JULIO", "AGOSTO", "SEPTIEMBRE", "OCTUBRE", "NOVIEMBRE", "DICIEMBRE"];
          
          // Obtener los meses que corresponden al período actual
          const mesesDelPeriodo = esPrimerSemestre ? mesesPrimerSemestre : mesesSegundoSemestre;
          ...
          if (inspeccion.meses && inspeccion.meses.size > 0) {
            for (const [mes, dataMes] of inspeccion.meses.entries()) {
              ...
              if (columnasPorMes[mes] && mesesDelPeriodo.includes(mes)) {
                ...
                worksheet.getCell(`${columna}${filaMesHeader}`).value = `MES: ${mes}`;
          
                // Insertar el nombre del inspector en la fila 6
                if (dataMes.inspector && dataMes.inspector.nombre) {
                  worksheet.getCell(`${columna}${filaNombreInspector}`).value = dataMes.inspector.nombre;
                  ...
                  if (dataMes.inspector.firma) {
                    ...
                    await this.insertarImagen(worksheet, dataMes.inspector.firma, rangoCelda);
                  }
                }
              }
          ...
          this.logger.error(`Error al llenar inspecciones por mes: ${error.message}`);
          throw error;
        }
      }

      private async llenarDatosDeInspeccion(worksheet: ExcelJS.Worksheet, inspeccion: FormularioInspeccionEmergencia) {
        try {
          ...
          if (inspeccion.meses && inspeccion.meses.size > 0) {
            for (const [mes, dataMes] of inspeccion.meses.entries()) {
              ...
              if (columnasMesCantidadEstado[mes] && mesesDelPeriodo.includes(mes)) {
                ...
                // Agregar el nombre del mes directamente (sabiendo que MES: ya existe)
              worksheet.getCell(`${columnasCE.cantidad}${filaMesHeader}`).value = `MES: ${mes}`;

                
                // Llenar datos de sistemas pasivos
                if (dataMes.inspeccionesActivos && dataMes.inspeccionesActivos.sistemasPasivos) {
                  const sistemasPasivos = dataMes.inspeccionesActivos.sistemasPasivos;
                  ...
                  for (const [sistema, fila] of Object.entries(filasSistemasPasivos)) {
                    if (sistemasPasivos[sistema]) {
                      // Llenar cantidad
                      worksheet.getCell(`${columnasCE.cantidad}${fila}`).value = sistemasPasivos[sistema].cantidad || '';
                      
                      // Llenar estado (✓, X, N/A)
                      worksheet.getCell(`${columnasCE.estado}${fila}`).value = sistemasPasivos[sistema].estado || '';
                    }
                  }
                }
                
                // Llenar datos de sistemas activos
                if (dataMes.inspeccionesActivos && dataMes.inspeccionesActivos.sistemasActivos) {
                  const sistemasActivos = dataMes.inspeccionesActivos.sistemasActivos;
                  ...
                  for (const [sistema, fila] of Object.entries(filasSistemasActivos)) {
                    if (sistemasActivos[sistema]) {
                      // Llenar cantidad
                      worksheet.getCell(`${columnasCE.cantidad}${fila}`).value = sistemasActivos[sistema].cantidad || '';
                      
                      // Llenar estado (✓, X, N/A)
                      worksheet.getCell(`${columnasCE.estado}${fila}`).value = sistemasActivos[sistema].estado || '';
                    }
                  }
                }
                
                // Agregar observaciones si existen
                if (dataMes.inspeccionesActivos && dataMes.inspeccionesActivos.observaciones) {
                  // Fila y columna para observaciones según la imagen (ajustar según sea necesario)
                  worksheet.getCell(`N11`).value = dataMes.inspeccionesActivos.observaciones;
                }
              }
            }
          }
        } catch (error) {
          this.logger.error(`Error al llenar datos de inspección: ${error.message}`);
          throw error;
        }
      }

      private async llenarInspeccionesExtintores(worksheet: ExcelJS.Worksheet, inspeccion: FormularioInspeccionEmergencia) {
        try {
          // Determinar si el mes pertenece al primer o segundo semestre
          const esPrimerSemestre = inspeccion.periodo === "ENERO-JUNIO";
          const mesesPrimerSemestre = ["ENERO", "FEBRERO", "MARZO", "ABRIL", "MAYO", "JUNIO"];
      ...
      worksheet.mergeCells(`S${filaHeader + 1}:U${filaHeader + 1}`);
            ...
            for (let i = filaInicio; i <= filaFin; i++) {
              ...
              for (let colIndex = 1; colIndex <= 21; colIndex++) { // columnas A-U
          ...
          if (inspeccion.meses && inspeccion.meses.size > 0) {
            for (const [mes, dataMes] of inspeccion.meses.entries()) {
              ...
              if (mesesDelPeriodo.includes(mes) && dataMes.inspeccionesExtintor && dataMes.inspeccionesExtintor.length > 0) {
                todosLosExtintores.push(...dataMes.inspeccionesExtintor);
              }
            }
          }
          
          this.logger.log(`Total de extintores a procesar: ${todosLosExtintores.length}`);
          ...
          while (extintorProcesado < todosLosExtintores.length) {
            ...
            const extintoresRestantes = todosLosExtintores.length - extintorProcesado;
            const cantidadAProcesar = Math.min(extintoresRestantes, tabla.capacidad);
            ...
            for (let i = 0; i < cantidadAProcesar; i++) {
              ...
              worksheet.getCell(`A${filaActual}`).value = extintor.fechaInspeccion || '';
              worksheet.getCell(`B${filaActual}`).value = extintor.codigo || '';
              worksheet.getCell(`C${filaActual}`).value = extintor.ubicacion || '';
              worksheet.getCell(`G${filaActual}`).value = extintor.inspeccionMensual || '';
              worksheet.getCell(`I${filaActual}`).value = extintor.manguera || '';
              worksheet.getCell(`K${filaActual}`).value = extintor.cilindro || '';
              worksheet.getCell(`M${filaActual}`).value = extintor.indicadorPresion || '';
              worksheet.getCell(`O${filaActual}`).value = extintor.gatilloChavetaPrecinto || '';
              worksheet.getCell(`Q${filaActual}`).value = extintor.senalizacionSoporte || '';
              worksheet.getCell(`S${filaActual}`).value = extintor.observaciones || '';
            }
          ...
          this.logger.log(`Procesado ${extintorProcesado} de ${todosLosExtintores.length} extintores`);
          this.logger.log(`Se crearon ${tablas.length - 2} tablas adicionales`);
          
        } catch (error) {
          this.logger.error(`Error al llenar inspecciones de extintores: ${error.message}`);
          throw error;
        }
      }
      ...
      async generateExcelSingle(inspeccion: FormularioInspeccionEmergencia): Promise<Buffer> {
        try {
