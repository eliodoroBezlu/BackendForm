...
export class ExcelManLiftService {
  ...
  private readonly logger = new Logger(ExcelManLiftService.name);

  constructor(private readonly configService: ConfigService) {
    this.templatePath =
      this.configService.get<string>('VEHICLE_EXCEL_TEMPLATE_PATH') ||
      path.join(process.cwd(), 'src', 'templates', 'ManLift.xlsx');
  }
  ...
  private async llenarCamposVerificacion(
    ...
    try {
      ...
      worksheet.getCell('K6').value = valores[7] || ''; // CAPACIDAD
      worksheet.getCell('K7').value = valores[8] || ''; // PLACA N| INTERNO
      worksheet.getCell('K8').value = valores[9] || ''; // RECORRRIDO
...
//if (insp.inspectionDate) worksheet.getCell(posiciones.inspector.fecha).value = insp.inspectionDate;
  ...
  async generateExcel(inspection: InspectionHerraEquipos): Promise<Buffer> {
    try {
      ...
      await this.llenarDatosManlift(worksheet, inspection);
