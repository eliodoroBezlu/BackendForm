...
export class ExcelCilindrosService {
  ...
  private readonly logger = new Logger(ExcelCilindrosService.name);

  constructor(private readonly configService: ConfigService) {
    this.templatePath =
      this.configService.get<string>('VEHICLE_EXCEL_TEMPLATE_PATH') ||
      path.join(process.cwd(), 'src', 'templates', 'Cilindros.xlsx');
  }

  /**
   * Códigos de template soportados: 1.02.P06.F20
   */
  ...
  private async llenarCamposVerificacion(
    ...
    try {
      ...
      worksheet.getCell('M6').value = valores[0] || ''; // Lugar de inspección
  ...
  private async llenarRespuestas(
    ...
    try {
      ...
      const allSections = [
        { id: 'section_0', startRow: 15, endRow: 19, name: 'ALMACENAMIENTO' },
        { id: 'section_1', startRow: 21, endRow: 35, name: '1.02.P06.F20' },
      ];
