...
@Injectable()
export class ExcelGruaRemotoService {
  private readonly templatePath: string;
  private readonly logger = new Logger(ExcelGruaRemotoService.name);

  constructor(private readonly configService: ConfigService) {
    this.templatePath =
      this.configService.get<string>('VEHICLE_EXCEL_TEMPLATE_PATH') ||
      path.join(process.cwd(), 'src', 'templates', 'GruaRemoto.xlsx');
  }
...
private async llenarCamposVerificacion(
  ...
  try {
    ...
    const cellMapping = [
      { cell: 'A5', value: valores[0], label: 'Nombre del operador' },
      { cell: 'A6', value: valores[1], label: 'Tag del puente grua ' },
      { cell: 'D5', value: valores[2], label: 'fecha' },
      { cell: 'D6', value: valores[3], label: 'turno' },
      { cell: 'G5', value: valores[4], label: 'hora' },
      { cell: 'G6', value: valores[5], label: 'grupo' }
    ];
...
private async llenarRespuestas(
  ...
  try {
    ...
    const sectionConfig = {
      ...
      'section_0': { startRow: 10, endRow: 17, name: 'INSPECCIÓN VISUAL' },
      'section_0_sub0': { startRow: 10, endRow: 17, name: 'Aspectos a Verificar' },    

      // Sección 1 y sus subsecciones
      'section_1': { startRow: 18, endRow: 38, name: 'Accesorios de izaje', hasSubsections: true },
      'section_1_sub0': { startRow: 20, endRow: 25, name: 'Eslinga sintetica' },
      'section_1_sub1': { startRow: 27, endRow: 32, name: 'grillete' },
      'section_1_sub2': { startRow: 34, endRow: 35, name: 'bolsa big bag' },
      'section_1_sub3': { startRow: 37, endRow: 38, name: 'kibble' },
      
      // Otras secciones si las hay
      'section_2': { startRow: 51, endRow: 55, name: 'Observaciones Finales' },
    };
