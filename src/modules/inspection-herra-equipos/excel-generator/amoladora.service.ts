...
export class ExcelAmoladoraService {
  ...
  private readonly logger = new Logger(ExcelAmoladoraService.name);
  ...
  /**
   * Códigos de template soportados: 1.02.P06.F39
   */
   ...
   */
  private async llenarDatosAmoladora(
    worksheet: ExcelJS.Worksheet,
    inspection: InspectionHerraEquipos,
  ) {
    try {
  ...
  async generateExcel(inspection: InspectionHerraEquipos): Promise<Buffer> {
    try {
      ...
      await this.llenarDatosAmoladora(worksheet, inspection);
