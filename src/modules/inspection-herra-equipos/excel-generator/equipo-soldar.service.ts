...
export class ExcelEquipoSoldarService {
  ...
  private readonly logger = new Logger(ExcelEquipoSoldarService.name);

  constructor(private readonly configService: ConfigService) {
    this.templatePath =
      this.configService.get<string>('VEHICLE_EXCEL_TEMPLATE_PATH') ||
      path.join(process.cwd(), 'src', 'templates', 'EquipoSoldar.xlsx');
  }

  /**
   * Códigos de template soportados: 1.02.P06.F42
   */
  ...
  private async llenarCamposVerificacion(
    ...
    try {
      ...
      worksheet.getCell('C6').value = valores[0] || ''; // Direccion gerencia
   ...
   */
  private async llenarDatosEquipoSoldar(
    worksheet: ExcelJS.Worksheet,
    inspection: InspectionHerraEquipos,
  ) {
    try {
  ...
  async generateExcel(inspection: InspectionHerraEquipos): Promise<Buffer> {
    try {
      ...
      await this.llenarDatosEquipoSoldar(worksheet, inspection);
