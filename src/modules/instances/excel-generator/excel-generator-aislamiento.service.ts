...
export class ExcelAislamientoervice {
  ...
  private readonly logger = new Logger(ExcelAislamientoervice.name);

  constructor(private readonly configService: ConfigService) {
    this.templatePath =
      this.configService.get<string>('ISOLATION_EXCEL_TEMPLATE_PATH') ||
      path.join(process.cwd(), 'src', 'templates', 'aislamiento.xlsx');
  }
  ...
  private async llenarConclusiones(
    ...
    worksheet.getCell('A118').value = instance.aspectosPositivos || '';
    worksheet.getCell('A121').value = instance.aspectosAdicionales || '';
  }
