...
export class ExcelElectricoCondicionesService {
  ...
  private readonly logger = new Logger(ExcelElectricoCondicionesService.name);

  constructor(private readonly configService: ConfigService) {
    this.templatePath =
      this.configService.get<string>('ISOLATION_EXCEL_TEMPLATE_PATH') ||
      path.join(process.cwd(), 'src', 'templates', 'electricoCondiciones.xlsx');
  }
  ...
  private async llenarSecciones(
    ...
    try {
      ...
      const sectionPositions = [
        ...
        { startRow: 87, name: 'F.  EQUIPO ELÉCTRICO PORTÁTIL' }, // Ajustar según la plantilla
        { startRow: 95, name: 'F.  EQUIPOS ELÉCTRICOS' }, // Ajustar según la plantilla
      ];
  ...
  private async llenarConclusiones(
    ...
    worksheet.getCell('A106').value = instance.aspectosPositivos || '';
    worksheet.getCell('A109').value = instance.aspectosAdicionales || '';
  }
