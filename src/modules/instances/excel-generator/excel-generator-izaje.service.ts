...
export class ExcelIzajeService {
  ...
  private readonly logger = new Logger(ExcelIzajeService.name);

  constructor(private readonly configService: ConfigService) {
    this.templatePath =
      this.configService.get<string>('ISOLATION_EXCEL_TEMPLATE_PATH') ||
      path.join(process.cwd(), 'src', 'templates', 'izaje.xlsx');
  }
  ...
  private async llenarConclusiones(
    ...
    worksheet.getCell('A132').value = instance.aspectosPositivos || '';
    worksheet.getCell('A135').value = instance.aspectosAdicionales || '';
  }
