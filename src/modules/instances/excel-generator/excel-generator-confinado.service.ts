...
@Injectable()
export class ExcelConfinadoService {
  private readonly templatePath: string;
  private readonly logger = new Logger(ExcelConfinadoService.name);

  constructor(private readonly configService: ConfigService) {
    this.templatePath =
      this.configService.get<string>('ISOLATION_EXCEL_TEMPLATE_PATH') ||
      path.join(process.cwd(), 'src', 'templates', 'confinado.xlsx');
  }
  ...
  private async llenarListaVerificacion(
    ...
    try {
      ...
      worksheet.getCell('C9').value = valores[5] || ''; // Fecha Inspección
      worksheet.getCell('H9').value = valores[6] || ''; // area
  ...
  private async llenarSecciones(
    ...
    try {
      ...
      const sectionPositions = [
        ...
        { startRow: 52, name: 'B. EMISOR DE PERMISO DE TRABAJO' }, // Ajustar según la plantilla
        { startRow: 59, name: 'C. PORTADOR DE PERMISO DE TRABAJO' },
        { startRow: 69, name: 'D.  SUPERVISOR DE AREA' }, // Ajustar según la plantilla
        { startRow: 87, name: 'E. ASISTENTE DE INGRESO' },
        { startRow: 100, name: 'F. ENTRANTES ESPACIOS CONFINADOS' }, // Ajustar según la plantilla
        {
          startRow: 114,
          name: 'F. EQUIPOS ELÉCTRICOS DENTRO UN ESPACIO CONFINADO',
        }, // Ajustar según la plantilla
        {
          startRow: 121,
          name: 'F. TRABAJOS EN CALIENTE EN ESPACIOS CONFINADOS',
        }, // Ajustar según la plantilla
      ];
  ...
  private async llenarConclusiones(
    ...
    worksheet.getCell('A131').value = instance.aspectosPositivos || '';
    worksheet.getCell('A134').value = instance.aspectosAdicionales || '';
  }
