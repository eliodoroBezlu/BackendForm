...
export class ExcelSustanciasService {
  ...
  private readonly logger = new Logger(ExcelSustanciasService.name);

  constructor(private readonly configService: ConfigService) {
    this.templatePath =
      this.configService.get<string>('ISOLATION_EXCEL_TEMPLATE_PATH') ||
      path.join(process.cwd(), 'src', 'templates', 'sustancias.xlsx');
  }
  ...
  private async llenarListaVerificacion(
    ...
    try {
      ...
      worksheet.getCell('C8').value = valores[3] || ''; // Superintendencia senior
      worksheet.getCell('H8').value = valores[4] || ''; // area fisica
      worksheet.getCell('C9').value = valores[5] || ''; // Fecha Inspección
      worksheet.getCell('H9').value = valores[6] || ''; // superintendencia
  ...
  private async llenarSecciones(
    ...
    try {
      ...
      const sectionPositions = [
        ...
        { startRow: 45, name: 'B. ALMACENAMIENTO' }, // Ajustar según la plantilla
        { startRow: 56, name: 'C. SEÑALIZACIÓN' },
        { startRow: 62, name: 'D. EQUIPOS DE EMERGENCIAS' }, // Ajustar según la plantilla
      ];
  ...
  private async llenarConclusiones(
    ...
    worksheet.getCell('A75').value = instance.aspectosPositivos || '';
    worksheet.getCell('A78').value = instance.aspectosAdicionales || '';
  }
