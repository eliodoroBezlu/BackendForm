...
export class ExcelCalienteService {
  ...
  private readonly logger = new Logger(ExcelCalienteService.name);

  constructor(private readonly configService: ConfigService) {
    this.templatePath =
      this.configService.get<string>('ISOLATION_EXCEL_TEMPLATE_PATH') ||
      path.join(process.cwd(), 'src', 'templates', 'trabajos_caliente.xlsx');
  }
  ...
  private async llenarEquipoInspeccion(
    ...
    try {
      ...
      this.logger.log('Equipo de inspección completado exitosamente');
    } catch (error) {
      this.logger.error(
  ...
  private async llenarSecciones(
    ...
    try {
      ...
      const sectionPositions = [
        { startRow: 33, name: 'A. TRABAJOS EN CALIENTE - GENERAL' },
        { startRow: 49, name: 'B. SOLDADURA CON ARCO ELÉCTRICO' }, // Ajustar según la plantilla
        { startRow: 71, name: 'C. OPERACIÓN DE OXICORTE' },
        { startRow: 99, name: 'D. ALMACENAMIENTO DE CILINDROS' }, // Ajustar según la plantilla
        { startRow: 116, name: 'E. USO DE AMOLADORA ELÉCTRICA' },
        { startRow: 146, name: 'F. TRABAJOS EN CALIENTE EN AREAS HÚMEDAS' },
        { startRow: 150, name: 'G. TRABAJOS EN CALIENTE ALTURAS' },
